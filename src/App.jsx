...
export default function App() {
  ...
  const [rows, setRows] = useState([
    {
      sv: { options: salesForceVariables },
      callHubvalue: { options: callHubValues },
    },
  ]);
    ...
    setRows((prevRows) => {
      ...
      if (selectedValue === "choose") {
        ...
        updatedRows[rowIndex].sv = {
          previousValue: prevRows[rowIndex].sv.previousValue
            ? prevRows[rowIndex].sv.actualValue
            : selectedValue,
          actualValue: "choose",
      ...
      for (let l = 0; l < updatedRows.length; l++) {
        ...
        const isAlreadyFound = updatedRows[l].sv.options.filter(
        ...
        if (updatedRows[i].sv?.actualValue == selectedValue) {
          for (let j = 0; j < updatedRows.length; j++) {
            if (j == rowIndex) {
              ...
              updatedRows[rowIndex].sv.options = updatedRows[
                rowIndex
              ].sv.options.filter((option) => option !== selectedValue);
            }
          }
        } else {
          updatedRows[i].sv.options = updatedRows[i].sv.options.filter(
    ...
    setRows((prevRows) => {
      ...
      if (selectedValue === "choose") {
        ...
        updatedRows[rowIndex].callHubvalue = {
          previousValue: prevRows[rowIndex].callHubvalue.previousValue
            ? prevRows[rowIndex].callHubvalue.actualValue
            : selectedValue,
          actualValue: "choose",
      ...
      for (let l = 0; l < updatedRows.length; l++) {
        ...
        const isAlreadyFound = updatedRows[l].callHubvalue.options.filter(
          (option) => option == replacement
        );
        if (isAlreadyFound == undefined || isAlreadyFound.length) {
          continue;
        ...
        if (updatedRows[i].callHubvalue?.actualValue == selectedValue) {
          for (let j = 0; j < updatedRows.length; j++) {
            if (j == rowIndex) {
              ...
              updatedRows[rowIndex].callHubvalue.options = updatedRows[
                rowIndex
              ].callHubvalue.options.filter(
                (option) => option !== selectedValue
              );
            }
          }
        } else {
          updatedRows[i].callHubvalue.options = updatedRows[
            i
          ].callHubvalue.options.filter((option) => option !== selectedValue);
        }
      }
  ...
  const AddAnotherField = () => {
    ...
    setRows((prevRows) => {
      ...
      return [
        ...prevRows,
        {
          sv: { options: salesForceVariablesCopy },
          callHubvalue: { options: callHubVariablesCopy },
        },
      ];
    });
  };
    ...
    <section>
      ...
      {rows.map((row, i) => {
        return (
          <div key={uuid()}>
            <select
              onChange={(e) => handleSfv(e, i)}
              value={row.sv.actualValue}
            >
            ...
            <button onClick={() => handleDelete(i)} className="delete-cta">
              <RiDeleteBin5Line />
            </button>
          </div>
        );
      ...
      <div className="cta-group">
        ...
        <button onClick={SubFieldMapper} className="submit-cta">
          Save and import lists
        </button>
      </div>
      {fieldMappers.length ? (
        <>
          <hr />
          {fieldMappers.map((field) => (
            <p key={uuid()} className="mapped-text">
              {field[0]} - {field[1]}
            </p>
          ))}
        </>
